...
export class PrivateKey {
    ...
    sign(msg: Uint8Array, scheme: Scheme): Signature {
        ...
        const q = Blst.hash_to_g2(msg, getSuite(scheme, 2), aug);
...
export class Signature {
    ...
    bytes(): Uint8Array {
        return Blst.p2_affine_compress(this.point);
    }
    ...
    aggregateVerify(scheme: Scheme, ...pairs: [PublicKey, Uint8Array][]): boolean {
        ...
        return Blst.core_aggregate_verify_g1(this.point, true, verifyPairs, true, true, getSuite(scheme, 2));
    }

    verify(scheme: Scheme, pk: PublicKey, msg: Uint8Array): boolean {
        const aug = scheme === "aug" ? pk.bytes() : undefined;
        return Blst.core_verify_pk_in_g1(pk._point, this.point, true, msg, getSuite(scheme, 2), aug);
    }

    valid(groupCheck: boolean = false): boolean {
        return !Blst.p2_affine_is_inf(this.point) && (!groupCheck || Blst.p2_affine_in_g2(this.point));
    }

    onCurve(): boolean {
        return Blst.p2_affine_on_curve(this.point);
    }

    equal(other: Signature): boolean {
        return Blst.p2_affine_is_equal(this.point, other.point);
    }
}
